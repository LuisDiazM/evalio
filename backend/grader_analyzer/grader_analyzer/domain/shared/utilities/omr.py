...
def preprocess_image(image_path):
    # Intentar leer la imagen con ruta absoluta/relativa
    ...
    thresh = cv2.adaptiveThreshold(
        gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 121, 10
    )
...
def find_filled_circles(circles, thresh):
    ...
    for x, y, r in circles:
        ...
        cv2.circle(
            mask, (int(x), int(y)), int(r), (255, 255, 255), -1
        )  # Usar 255 para blanco
...
if __name__ == "__main__":
    # Usar la imagen por defecto que está junto a este script
    script_dir = Path(__file__).resolve().parent
    image_path = script_dir / "exam-684037c7b4a896fcb04dcace-37671441.jpeg"

    print(f"Script dir: {script_dir}")
    print(f"Usando ruta de imagen: {image_path}")
    print(f"Existe? {image_path.exists()}")

    try:
        output_file = script_dir / "output.jpg"
        result = grade_exam(image_path, str(output_file))
        print("Respuestas detectadas:", result.get("responses", []))
        print("Salida guardada en:", result.get("output"))
